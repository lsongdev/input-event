...
function Mouse(device){
  ...
  this.on('data', function(ev, data){
    ...
    if(data.length == 3){
      ...
      if(ev.type == 8) this.emit((ev.code == 0 && ev.value == 0) ? 'reset':'move', ev);
      if(ev.type & MOUSE_BUTTON.BTN_LEFT  ||
         ev.type & MOUSE_BUTTON.BTN_RIGHT ||
         ev.type & MOUSE_BUTTON.BTN_MIDDLE
